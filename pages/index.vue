  ...
  <div id="top" class="app">
      ...
      <div class="apex flex p-2 justify-between">
        <div class="item-count">
          <div v-if="searchQuery" class="search-count">
            {{ items.length }} <span v-if="items.length > 1">items</span
    ...
    <div class="container">
      ...
      <main class="px-2">
                ...
                <fieldset>
                  <div class="instructions leading-normal pb-2">
                    <div class="instructions__content px-2 py-3">
                      <p>
                        To add an item, use the file input below to select a
                        ...
                        <div class="header-wrap">
                          <div class="item__header relative">
                            ...
                            <div class="item__media opacity-100">
                              ...
                              >
                                <div
                                  v-for="(media, index) in newItemMedia"
                                  :key="'new-item-media-' + index"
                                  class="media-shell"
                                >
                                  <img
                                    v-if="media.type === 'GraphImage'"
                                    :src="media.url"
                                    alt
                                  />
                                  <img
                                    v-else-if="media.type === 'GraphVideo'"
                                    :src="media.url"
                                    alt
                                  />
                                </div>
        ...
        <div v-if="items.length" id="items" ref="items" class="items pb-8">
          <item
...
export default {
  ...
  methods: {
        ...
        reader.addEventListener(
          ...
          function(evt) {
            ...
            if (
              ...
              const sidecars = this.jsonData.node.edge_sidecar_to_children
                .edges;
    ...
    addItem() {
      if (!this.newItemContent || !this.$refs.newItemContent.textContent) {
        return;
      }
      this.saveItems();
    },
        ...
        entries.forEach(entry => {
          if (entry.intersectionRatio > 0) {
            if (this.$store.state.itemsRemaining === false) {
              ...
              const lastItemId = this.items[this.items.length - 1]._id;
